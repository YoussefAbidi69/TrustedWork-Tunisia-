...
export class MarketplaceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.applyForm = this.fb.group({
      coverLetter: ['', [Validators.required, Validators.minLength(50), Validators.maxLength(500)]],
  ...
  private enrichAndSet(offers: JobOffer[]): void {
    ...
    rec$.subscribe((recs) => {
      ...
      if (role === 'FREELANCER' && uid != null) {
        const matchCalls = offers.map((j) =>
          this.jobBoard.getJobMatches(j.id).pipe(catchError(() => of([] as MatchFreelancerRow[])))
        );
        forkJoin(matchCalls).subscribe((rowsPerJob) => {
          this.feedJobs = offers.map((job, i) => {
            const row = rowsPerJob[i]?.find((m) => m.freelancerId === uid);
            const score = row?.totalMatchScore ?? recMap.get(job.id) ?? null;
            return this.mapOffer(job, score);
          });
          this.finishLoad();
        });
        return;
    ...
    this.jobBoard.getJobById(job.id).subscribe({
      next: (detail) => {
        ...
        if (this.auth.getCurrentAuthUser()?.role?.toUpperCase() === 'FREELANCER' && uid != null) {
          this.jobBoard.getJobMatches(job.id).subscribe({
            next: (rows) => {
              const row = rows.find((m) => m.freelancerId === uid);
              this.panelMatchScore = row?.totalMatchScore ?? job.matchScore;
            },
            error: () => {}
          });
          this.jobBoard
