...
export class AuthService {
  ...
  login(payload: LoginPayload, rememberMe: boolean = true): Observable<AuthResponse> {
    return this.api.post<AuthResponse>('/auth/login', payload, undefined, this.api.getAuthUrl()).pipe(
  ...
  verifyTwoFactor(payload: VerifyTwoFactorPayload, rememberMe?: boolean): Observable<AuthResponse> {
    ...
    return this.api.post<AuthResponse>('/auth/verify-2fa', payload, undefined, this.api.getAuthUrl()).pipe(
      tap((response: AuthResponse) => {
  ...
  register(payload: unknown): Observable<unknown> {
    return this.api.post('/auth/register', payload, undefined, this.api.getAuthUrl());
  }

  forgotPassword(payload: ForgotPasswordPayload): Observable<any> {
    return this.api.post('/auth/forgot-password', payload, undefined, this.api.getAuthUrl());
  }

  resetPassword(payload: ResetPasswordPayload): Observable<any> {
    return this.api.post('/auth/reset-password', payload, undefined, this.api.getAuthUrl());
  }
  ...
  getRole(): string | null {
    return this.getCurrentAuthUser()?.role || null;
  }
