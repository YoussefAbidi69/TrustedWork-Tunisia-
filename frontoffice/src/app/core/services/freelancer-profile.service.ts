...
export class FreelancerProfileService {
  ...
  private readonly USER_BASE_URL = '/api';
  ...
  /**
   * Tous les avis du profil (VISIBLE + HIDDEN + FLAGGED).
   * Réservé au propriétaire connecté — appelle /all (FREELANCER role requis).
   */
  getReviews(profileId: number): Observable<ProfileReview[]> {
    return this.http.get<ProfileReview[]>(`${this.BASE_URL}/reviews/profiles/${profileId}/all`);
  }

  /**
   * Uniquement les avis VISIBLE — pour la page publique du profil.
   */
  ...
  hideReview(reviewId: number): Observable<void> {
    return this.http.patch<void>(`${this.BASE_URL}/reviews/${reviewId}/hide`, {});
  }

  restoreReview(reviewId: number): Observable<void> {
    return this.http.put<void>(`${this.BASE_URL}/reviews/${reviewId}/restore`, {});
  }
