...
export class MilestoneService {
  ...
  private endpoint = '/v1/milestones';

  constructor(private api: ApiService) {}

  getAll(page: number = 0, size: number = 10): Observable<any> {
    return this.api.get<any>(this.endpoint, { params: { page, size } });
  }
  ...
  updateRejectedDeadline(id: number, newDeadline: string): Observable<Milestone> {
    return this.api.patch<Milestone>(`${this.endpoint}/${id}/deadline`, {}, { params: { newDeadline } });
  }
