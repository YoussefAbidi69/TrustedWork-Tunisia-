...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.email = this.authService.getCurrentAuthUser()?.email || 'client@trustedwork.com';
  ...
  async onSubmit() {
    ...
    try {
      ...
      // Détecter automatiquement le mode simulation si le backend retourne un sim_secret
      const isSimulated = intent.clientSecret?.startsWith('sim_secret');

      if (this.simulationMode || isSimulated) {
        ...
        // Confirmer côté backend directement (simulation)
        this.stripeService.confirmPayment(intent.paymentIntentId, this.contractId).subscribe({
          next: () => {
            this.router.navigate(['/app/finance/payment-result'], { 
