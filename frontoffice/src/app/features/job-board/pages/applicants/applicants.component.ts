...
export class ApplicantsComponent implements OnInit, OnDestroy {
  ...
  get rows() { return this.filteredApplicants; }

  /* ── Filter & Sort state ── */
  statusFilter: string = 'All';
  matchScoreFilter: string = 'Any';
  sortAscending: boolean = false;

  get filteredApplicants(): JobApplication[] {
    ...
    // Match Score filter
    if (this.matchScoreFilter !== 'Any') {
      const minScore = parseInt(this.matchScoreFilter.replace('%+', ''));
      filtered = filtered.filter(app => (app.matchScore?.totalScore || 0) >= minScore);
    }
  ...
  chatPeerId = 0;
  chatPeerLabel = '';
  chatAttachedFile: File | null = null;
  ...
  private loadPredictions(): void {
    if (!this.applicants.length) return;
    this.applicants.forEach(app => {
      this.jobBoard.getSuccessPrediction({
        jobOfferId: this.jobId,
        freelancerId: app.freelancerId
      }).subscribe({
        next: (pred) => {
          this.predictions[app.freelancerId] = pred;
        },
        error: () => {
          // silently ignore if a prediction is not available
        }
      });
    });
  }
  ...
  get pending(): number { return this.applicants.filter(r => r.status === 'PENDING').length; }
  ...
  openChat(app: JobApplication): void {
    ...
    this.chatPeerLabel = app.freelancerName ? app.freelancerName : `Freelancer #${app.freelancerId}`;
    this.chatOpen = true;
    this.chatInput = '';
