...
export class PublicProfileComponent implements OnInit {
  ...
  private loadPublicProfile(profileId: number): void {
    ...
    this.freelancerProfileService.getProfileByUserId(profileId).pipe(
      catchError(() => {
        this.isLoading = false;
      ...
      next: (profile: FreelancerProfile | null) => {
        ...
        forkJoin({
          ...
          next: (res: any) => {
            ...
            // Profil public : seuls les avis VISIBLE (pas FLAGGED ni HIDDEN)
            // Les avis suspects ne sont jamais montrés aux visiteurs
            this.reviews = (res.reviews || []).filter(
              (r: ProfileReview) => r.status === 'VISIBLE' && !r.flagged
            );
