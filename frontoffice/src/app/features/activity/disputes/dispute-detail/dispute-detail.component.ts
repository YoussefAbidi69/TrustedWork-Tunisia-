...
export class DisputeDetailComponent implements OnInit {
  ...
  loadDispute(): void {
    ...
    this.disputeService.getById(this.disputeId).subscribe({
      ...
      error: (err) => { this.error = 'Impossible de charger le litige.'; this.loading = false; console.error(err); }
    });
  }
  ...
  loadEvidence(): void {
    this.disputeService.listEvidence(this.disputeId).subscribe({
      ...
      error: (err) => console.error('Evidence load error:', err)
    });
  }
  ...
  submitAssign(): void {
    ...
    this.disputeService.assign(this.disputeId, this.arbitreId || undefined).subscribe({
      ...
      error: (err) => { this.assigning = false; alert('Erreur: ' + (err.error?.message || '')); }
    });
  }
  ...
  submitResolve(): void {
    ...
    this.disputeService.resolve(this.disputeId, this.resolveForm).subscribe({
      ...
      error: (err) => { this.resolving = false; alert('Erreur: ' + (err.error?.message || '')); }
    });
  }
