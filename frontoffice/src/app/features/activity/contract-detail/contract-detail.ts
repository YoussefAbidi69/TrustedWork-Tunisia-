...
export class ContractDetailComponent implements OnInit {
    ...
    this.signatureService.finalizeContract(this.contract.id).subscribe({
      next: () => {
        this.signatureService.sendSignatureRequests(this.contract!.id!).subscribe({
          next: (signingRequests: any) => {
            console.log('[ContractDetail] sendSignatureRequests response:', signingRequests);
            this.captureFreelancerIdFromSigningRequests(signingRequests);
  ...
  private captureFreelancerIdFromSigningRequests(response: any): void {
    if (!this.contract?.id || !response) return;
    const requests = Array.isArray(response) ? response : [response];
    const freelancerCin = this.contract.freelancerCin;
    const freelancerReq = requests.find((r: any) =>
      r?.role === 'FREELANCER' || r?.userRole === 'FREELANCER' ||
      r?.cin === freelancerCin || r?.userCin === freelancerCin ||
      r?.signerCin === freelancerCin
    );
    const freelancerUserId =
      freelancerReq?.userId ?? freelancerReq?.signerUserId ??
      freelancerReq?.user?.id ?? freelancerReq?.user?.userId;
    if (freelancerUserId) {
      localStorage.setItem(`contract_${this.contract.id}_freelancer_id`, String(freelancerUserId));
      console.log('[ContractDetail] freelancerId stored from signing requests:', freelancerUserId);
    } else {
      console.log('[ContractDetail] freelancerId not found in signing requests. Available fields:', requests.map((r: any) => Object.keys(r || {})));
    }
  }
