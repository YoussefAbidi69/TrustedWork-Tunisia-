...
public interface ProfileReviewRepository extends JpaRepository<ProfileReview, Long> {
    ...
    /**
     * Tous les avis d'un profil, toutes statuses confondues (VISIBLE, HIDDEN, FLAGGED).
     * Utilisé exclusivement par le propriétaire du profil (dashboard).
     */
    List<ProfileReview> findByProfileIdOrderByReviewedAtDesc(Long profileId);
    ...
    /**
     * Compte les avis avec sentiment NEGATIVE encore visibles (non masqués).
     * Utilisé pour la détection automatique de suspension.
     */
    @Query("""
           SELECT COUNT(r)
           FROM ProfileReview r
           WHERE r.profile.id = :profileId
             AND r.sentimentLabel = 'NEGATIVE'
             AND r.status = 'VISIBLE'
           """)
    long countNegativeVisibleReviews(@Param("profileId") Long profileId);
}