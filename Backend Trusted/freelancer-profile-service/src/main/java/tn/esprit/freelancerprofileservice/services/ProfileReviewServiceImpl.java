...
public class ProfileReviewServiceImpl implements IProfileReviewService {
    ...
    public ReviewResponse addReview(Long profileId, AddReviewRequest request) {
        ...
        if (!"UNKNOWN".equals(sentiment.sentiment())) {
            ...
            // En fallback, sentimentLabel reste UNKNOWN (déjà dans sentiment.sentiment())
        }
        ...
        // Recalcul du score de réputation après chaque nouvel avis
        recalculateReputationScore(profile);

        // Vérification suspension automatique si trop d'avis négatifs
        checkAndSuspendIfTooManyNegative(profile);
    ...
    @Override
    public List<ReviewResponse> getAllReviews(Long profileId) {
        ensureProfileExists(profileId);
        return reviewRepository
                .findByProfileIdOrderByReviewedAtDesc(profileId)
                .stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public void hideReview(Long reviewId) {
        ...
        // Recalcul du score de réputation : l'avis masqué ne compte plus
        recalculateReputationScore(review.getProfile());
    }
    ...
    public ReviewResponse restoreReview(Long reviewId) {
        ...
        // Recalcul du score de réputation : l'avis restauré redevient actif
        recalculateReputationScore(review.getProfile());
    ...
    // =========================================================
    // MÉTHODES PRIVÉES — Logique IA & Réputation
    // =========================================================

    /**
     * Recalcule le reputationScore du profil.
     * Formule : moyenne des ratings des avis VISIBLE uniquement × 20
     * Résultat entre 0 et 100.
     */
    private void recalculateReputationScore(FreelancerProfile profile) {
        Double avgRating = reviewRepository.findAverageRatingByProfileId(profile.getId());
        double score = avgRating != null ? Math.round(avgRating * 20.0 * 10.0) / 10.0 : 0.0;
        profile.setReputationScore(score);
        profileRepository.save(profile);
        log.info("[REPUTATION] Score recalculé pour profil {} → {}", profile.getId(), score);
    }

    /**
     * Vérifie si le profil doit être suspendu automatiquement.
     * Règle : >= 3 avis NEGATIVE non-masqués → suspension + email.
     */
    private void checkAndSuspendIfTooManyNegative(FreelancerProfile profile) {
        int threshold = 3;
        long negativeCount = reviewRepository.countNegativeVisibleReviews(profile.getId());

        if (negativeCount >= threshold && !Boolean.TRUE.equals(profile.getSuspended())) {

            // 1. Suspendre le profil freelancer (profile DB)
            profile.setSuspended(true);
            profileRepository.save(profile);

            // 2. Suspendre le compte utilisateur via user-service
            if (profile.getUserId() != null) {
                userClient.autoSuspendUser(
                        profile.getUserId(),
                        "Suspension automatique IA — " + negativeCount + " avis négatifs détectés"
                );
            }

            log.warn("[AUTO-SUSPENSION] Profil {} suspendu automatiquement ({} avis négatifs)",
                    profile.getId(), negativeCount);

            // 3. Email au freelancer (redirigé vers testRecipient = omsehli@gmail.com)
            emailService.sendAutoSuspensionEmail(
