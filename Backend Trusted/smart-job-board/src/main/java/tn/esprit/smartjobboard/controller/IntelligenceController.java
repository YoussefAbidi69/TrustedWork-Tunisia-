...
public class IntelligenceController {
    ...
    @GetMapping("/market/salary-insights")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<SalaryInsightDto>> salaryInsights() {
        return ResponseEntity.ok(marketAnalyticsService.computeSalaryInsights());
    }

    @GetMapping("/market/forecast")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<MarketForecastDto>> marketForecast() {
        return ResponseEntity.ok(marketAnalyticsService.computeForecast());
    }

    @GetMapping("/market/categories")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<MarketCategoryDto>> categories(
            @RequestParam(required = false) List<String> skills
    ) {
        return ResponseEntity.ok(marketAnalyticsService.computeCategoriesForSkills(skills));
    }

    @GetMapping("/market/overview")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<MarketOverviewDto> marketOverview() {
        return ResponseEntity.ok(marketAnalyticsService.computeOverviewMetrics());
    }
