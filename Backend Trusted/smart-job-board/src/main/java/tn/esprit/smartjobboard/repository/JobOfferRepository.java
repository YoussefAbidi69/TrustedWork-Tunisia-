...
public interface JobOfferRepository extends JpaRepository<JobOffer, Long>, JpaSpecificationExecutor<JobOffer> {
    ...
    @Query("SELECT DISTINCT j FROM JobOffer j LEFT JOIN FETCH j.extractedSkills WHERE j.status = :status")
    List<JobOffer> findByStatusWithExtractedSkills(@Param("status") JobOfferStatus status);
