...
public class TeamProjectServiceImpl implements ITeamProjectServices {
    ...
    public TeamProject createProject(Long agencyId, Long creatorMemberId, TeamProject project) {
        ...
        AgencyMember creator = agencyMemberRepository.findByAgencyIdAndUserId(agencyId, creatorMemberId)
                .orElseThrow(() -> new RuntimeException("Agency Member not found for this user"));
    ...
    public TeamProject updateProjectProgress(Long projectId) {
        ...
        long completedTasks = tasks.stream()
                .filter(task -> task.getStatus() == TaskStatus.TERMINE)
                .count();
    ...
    @Override
    public TeamProject assignMembers(Long projectId, List<Long> memberIds, Long requesterId) {
        TeamProject project = teamProjectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("Project not found"));
        
        AgencyMember requester = agencyMemberRepository.findByAgencyIdAndUserId(project.getAgency().getId(), requesterId)
                .orElseThrow(() -> new RuntimeException("Requester member not found"));

        if (!requester.getAgency().getId().equals(project.getAgency().getId()) || requester.getRole() != MemberRole.LEAD) {
            throw new RuntimeException("Only a LEAD can assign members");
        }

        List<AgencyMember> membersToAssign = agencyMemberRepository.findAllById(memberIds);
        if (project.getAssignedMembers() == null) {
            project.setAssignedMembers(new java.util.HashSet<>());
        }
        for (AgencyMember m : membersToAssign) {
            if (m.getAgency().getId().equals(project.getAgency().getId())) {
                project.getAssignedMembers().add(m);
            }
        }
    ...
    @Override
    public TeamProject removeMember(Long projectId, Long memberId, Long requesterId) {
        TeamProject project = teamProjectRepository.findById(projectId)
                .orElseThrow(() -> new RuntimeException("Project not found"));
        ...
        if (!requester.getAgency().getId().equals(project.getAgency().getId()) || requester.getRole() != MemberRole.LEAD) {
            throw new RuntimeException("Only a LEAD can remove members");
        }

        project.getAssignedMembers().removeIf(m -> m.getId().equals(memberId));
        return teamProjectRepository.save(project);
    }
}