...
import tn.esprit.userservice.repository.IAgencyJoinRequestRepository;
...
public class AgencyMemberServiceImpl implements IAgencyMemberServices {
    ...
    @Override
    public void removeMember(Long agencyId, Long userIdToRemove, Long requesterId) {
        // Enforce LEAD check
        AgencyMember requester = agencyMemberRepository.findByAgencyIdAndUserId(agencyId, requesterId)
                .orElseThrow(() -> new RuntimeException("You are not a member of this agency"));
        if (requester.getRole() != MemberRole.LEAD) {
            throw new RuntimeException("Only the agency LEAD can remove members");
        }

        // Check if removing self
        if (requesterId.equals(userIdToRemove)) {
            throw new RuntimeException("You cannot remove yourself this way");
        }

        AgencyMember memberToRemove = agencyMemberRepository.findByAgencyIdAndUserId(agencyId, userIdToRemove)
                .orElseThrow(() -> new RuntimeException("User is not a member of this agency"));
                
        agencyMemberRepository.delete(memberToRemove);
    }

    @Override
    @org.springframework.transaction.annotation.Transactional
    public void quitAgency(Long agencyId, Long userId) {
        AgencyMember member = agencyMemberRepository.findByAgencyIdAndUserId(agencyId, userId)
                .orElseThrow(() -> new RuntimeException("You are not a member of this agency"));

        // Guard: LEAD cannot quit — must delete the agency
        if (member.getRole() == MemberRole.LEAD) {
            throw new RuntimeException("As the agency owner (LEAD), you cannot quit. You must delete the agency instead.");
        }

        // Clean up any pending join requests the user had for this agency
        joinRequestRepository.deleteByAgencyIdAndRequesterId(agencyId, userId);

        agencyMemberRepository.delete(member);
    }
